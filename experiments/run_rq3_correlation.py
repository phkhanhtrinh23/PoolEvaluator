...
Dawid--Skene) inflates estimated accuracy as correlation grows, while PoolEval-SQL's
provenance-grouped model stays close to truth; and (right) PoolEval-SQL's reported
...
def main(seeds=6):
    ...
    print(f"\nRQ3: correlation sweep (mean over {seeds} seeds). As collusion grows, "
          "B3 inflates accuracy; PoolEval-SQL stays accurate and M_eff drops.")
    print("-" * 76)
    print(f"{'collusion':>10s}{'B3 MAE':>10s}{'PoolEval-SQL MAE':>18s}"
          f"{'M_eff':>10s}{'PoolEval-SQL bias':>18s}")
    table = []
    for c in levels:
        b3m, pem, meff, peb = [], [], [], []
        ...
        table.append(row)
        print(f"{c:10.2f}{row['b3_mae']:10.2f}{row['pe_mae']:18.2f}"
              f"{row['meff']:10.1f}{row['pe_bias']:+18.2f}")
