# Collision-Aware Formulation: Complete Real-Data Rerun, 2026-08-14
...
## 2026-08-14 rerun summary

This rerun used the same four real held-out datasets, 500 paired bootstrap
resamples, and seed 20260814. For each target, model priors came from its
train/meta data while $\gamma_g$ and $\beta_0$ came from the other three real
datasets. Target gold was used only after fitting.

| Dataset | Prior MAE | Old MAE | Plain MAE | Case-3 MAE | Old Kendall | Case-3 Kendall |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Spider | **3.73** | 13.38 | 19.59 | 11.93 | **0.71** | 0.66 |
| SQLFlow | **3.82** | **11.28** | 16.38 | 12.01 | 0.72 | 0.72 |
| BIRD | **3.47** | 17.20 | **16.32** | 17.22 | 0.60 | **0.69** |
| BIRD-MiniDev | **2.48** | 13.80 | 20.90 | 3.85 | 0.58 | **0.63** |
| Mean | **3.37** | 13.91 | 18.30 | 11.25 | 0.65 | **0.67** |

For $\Delta=\text{case 3}-\text{old}$, paired MAE intervals were:

| Dataset | Mean $\Delta$ | 95% interval |
| --- | ---: | ---: |
| Spider | -1.55 | [-3.17, 0.07] |
| SQLFlow | +0.65 | [-0.05, 1.17] |
| BIRD | -0.09 | [-0.84, 0.45] |
| BIRD-MiniDev | -8.84 | [-14.75, 0.31] |

No interval excludes zero. Case 3 is better on average but not uniformly or
significantly better per dataset. Plain binary EM produced negative Kendall on
SQLFlow (-0.67), BIRD (-0.47), and BIRD-MiniDev (-0.49); case 3 repaired these
to 0.72, 0.69, and 0.63.

## Calibration, convergence, and gamma diagnostics

| Dataset | Actual pseudo-label accuracy | Estimated $\beta$ | Iterations | Source $\gamma_g$ range |
| --- | ---: | ---: | ---: | ---: |
| Spider | 0.780 | 0.922 | 18 | 0.408–0.603 |
| SQLFlow | 0.487 | 0.798 | 61 | 0.517–0.659 |
| BIRD | 0.453 | 0.842 | 52 | 0.424–0.653 |
| BIRD-MiniDev | 0.327 | 0.257 | 93 | 0.471–0.656 |

Without the beta anchor, estimates reached approximately 1 on Spider, SQLFlow,
and BIRD and approximately 0 on BIRD-MiniDev. The anchor prevents boundary
degeneracy but does not fully calibrate beta. Oracle target gamma MAEs were
11.22, 12.93, 16.75, and 5.85 respectively, so gamma transfer error alone does
not explain remaining error.

## Post-EM LLM-judge rerun

Saved real gpt-5-mini verdicts were applied after case-3 EM, pseudo-labels and
$C$ were updated, and EM was rerun.

| Dataset | Before MAE | After 12 MAE | Before Kendall | After Kendall |
| --- | ---: | ---: | ---: | ---: |
...
For $\Delta=\text{after}-\text{before}$:

| Dataset | Mean MAE $\Delta$ | 95% interval | Mean Kendall $\Delta$ | 95% interval |
| --- | ---: | ---: | ---: | ---: |
| Spider | -0.94 | [-1.73, -0.25] | -0.026 | [-0.240, 0.183] |
| SQLFlow | -0.51 | [-1.08, -0.09] | +0.000 | [-0.134, 0.135] |
| BIRD | -0.71 | [-1.41, -0.11] | +0.006 | [-0.183, 0.224] |
| BIRD-MiniDev | -0.21 | [-0.73, 0.51] | -0.017 | [-0.136, 0.091] |

The judge changed 7, 8, 11, and 9 labels and issued 0, 1, 2, and 4 rejections.
Judge accuracies were 0.42, 0.08, 0.33, and 0.17. MAE gains were supported on
Spider, SQLFlow, and BIRD, but no ranking gain was supported. BIRD-MiniDev Top1
fell from 1 to 0 after budgets 9 and 12.

The saved verdicts used the old-method ambiguity order. This is a real-verdict
replay after case-3 EM, not new acquisition from the case-3 posterior.

## Rerun conclusion

Collision-aware EM repairs the plain binary likelihood and improves average MAE
over old PoolEval, but is not uniformly better. Post-EM judging adds calibration
gains without reliable ranking gains and can harm Top1. The train/source prior
remains the strongest absolute accuracy estimator on all four datasets.
