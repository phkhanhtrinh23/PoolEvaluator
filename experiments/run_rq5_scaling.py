...
quality for PoolEval vs the strongest independent baseline (B1). PoolEval's
...
def main(seeds=6):
    ...
    print("-" * 70)
    print(f"{'M':>4s}{'B1 Flip':>12s}{'PoolEval Flip':>16s}"
          f"{'B1 Kend':>12s}{'PoolEval Kend':>16s}")
    table = []
    for M in Ms:
        b1f, pef, b1k, pek = [], [], [], []
        ...
        table.append(row)
        print(f"{M:>4d}{row['b1_flip']:12.3f}{row['pe_flip']:16.3f}"
              f"{row['b1_kend']:12.2f}{row['pe_kend']:16.2f}")
