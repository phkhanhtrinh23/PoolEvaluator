...
from pooleval.config import load_manifest
...
def test_every_model_has_supported_backend():
    ...
    for task in ("text2sql", "image", "node"):
        assert {spec.backend for spec in load_manifest(task)} <= supported
...
def test_all_pyg_architectures_forward_when_optional_runtime_is_present():
    ...
    for spec in load_manifest("node"):
        if spec.backend == "pyg":
...
def test_external_graph_checkpoints_use_pipeline_runner_hook():
    ...
    spec = next(spec for spec in load_manifest("node") if spec.backend == "external")
